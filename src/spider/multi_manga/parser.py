...
class MangaParser(BaseMangaParser):
    def _parse_html(self, soup):
        ...
        for tag in _tags:
            ...
            if tag.next_element.get_text(strip=True).lower() == 'теги':
                tags['genres'] = [t.get_text(strip=True) for t in tag.select('a.tag')]
                
            if tag.next_element.get_text(strip=True).lower() == 'автор':
                tags['author'] = tag.select_one('a.tag').get_text(strip=True) if tag.select_one('a.tag') else None
                
            if tag.next_element.get_text(strip=True).lower() == 'язык':
...
class PageParser(BasePageParser):
    def _parse_html(self, soup):
        ...
        for item in soup.select('div#dle-content div.gallery'):
            total += 1
