...
class HitomiSpider(BaseSpider):
    ...
    async def total_pages(self) -> int:
        ...
        response = await self.http.get(
            urljoin(self.BASE_URL, self.PAGE_URL.format(page = 99999)), # Число 99999 так-как при огромном количестве страниц сайт по просту возращает последнюю страницу 
            "read",
        ...
        if response is None:
            ...
            page_links = [x for x in soup.select("section.pagination a") if x.get_text(strip=True).isdigit()]
            try:
    ...
    async def pages(self, start_page = None):
        max_pages = await self.total_pages
        page_parser = HitomiPageParser(self.BASE_URL, self.features)
