...
class FindService:
    ...
    async def get_pages_by_genre(self, genre_id: int, page: int, per_page: int | None = None) -> tuple[int, list[BaseManga]]:
        """
        ...
        async with self.manager.Session() as session:
            base_query = select(
                ...
                GenreManga.genre_id == genre_id
            )
            async def scalars_manga():
                query = base_query.options(
                    selectinload(GenreManga.manga)
                ).offset((page - 1) * (per_page)).limit(
                    per_page
                )
            ...
            total, result = await asyncio.gather(
                session.scalar(select(func.count()).select_from(base_query)),
                scalars_manga()
            )
            
            return math.ceil((total or 0) / per_page), result
    
    async def get_pages_by_author(self, author_id: int, page: int, per_page: int | None = None) -> tuple[int, list[BaseManga]]:
        """
        ...
        async with self.manager.Session() as session:
            ...
            async def scalars_manga():
                query = base_query.offset((page - 1) * (per_page)).limit(
                    per_page
                )
                mangas = await session.scalars(query)
                if not mangas:
    ...
    async def get_pages_by_language(self, language_id: int, page: int, per_page: int | None = None) -> tuple[int, list[BaseManga]]:
        """
