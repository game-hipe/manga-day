...
def setup_frontend(manager: MangaManager, spider: SpiderManager) -> FastAPI:
    ...
    app.include_router(setup_user(port=config.api.backend_port))
...
async def start_frontend(manager: MangaManager, spider: SpiderManager) -> None:
    app = setup_frontend(manager, spider)
